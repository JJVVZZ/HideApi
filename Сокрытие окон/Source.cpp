...
void RestoreWindow(HWND hwnd)
{
	...
	// Если окно свернуто — развернём
	if (IsIconic(hwnd)) {
		ShowWindow(hwnd, SW_RESTORE); // Разворачивает окно
	}
	else {
		ShowWindow(hwnd, SW_SHOW);    // Показывает, если просто было скрыто
	}

	SetForegroundWindow(hwnd);       // Возвращает фокус
